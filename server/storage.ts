...
export async function getChannels(playlistId?: number) {
  ...
  if (playlistId) {
    ...
    .where(eq(schema.channels.playlistId, playlistId))
...
export async function getChannelsByCategory(categoryId: number, playlistId?: number) {
  ...
  const conditions = [eq(schema.channels.categoryId, categoryId)];
  if (playlistId) {
    conditions.push(eq(schema.channels.playlistId, playlistId));
  }
...
export async function searchChannels(searchTerm: string, playlistId?: number) {
  ...
  const conditions = [like(schema.channels.name, `%${searchTerm}%`)];
  if (playlistId) {
    conditions.push(eq(schema.channels.playlistId, playlistId));
  }
...
export async function getProgramsByChannelEpgId(channelEpgId: string, startTime?: Date, endTime?: Date) {
  ...
  const conditions: any[] = [eq(schema.programs.channelEpgId, channelEpgId)];
  
  if (startTime) {
    conditions.push(sql`${schema.programs.startTime} >= ${startTime}`);
  }
  
  if (endTime) {
    conditions.push(sql`${schema.programs.endTime} <= ${endTime}`);
  }
