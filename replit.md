...
- **macOS Development**: Requires Node.js v20 LTS (v24 has ENOTSUP socket errors). Server uses PORT environment variable (default 3000 for Mac, 5000 for Linux production). Use `PORT=3000 npm run dev` on Mac. Database is optional for Simple Player mode - app works completely offline with localStorage.
