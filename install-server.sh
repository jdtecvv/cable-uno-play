#!/bin/bash
#==============================================================================
# Cable Uno Play - Script de Instalación Automática para Servidor Linux
# Compatible con: Ubuntu 20.04+, Debian 11+, CentOS 8+
#==============================================================================

set -e  # Salir si hay errores

# Colores para output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Funciones de utilidad
print_header() {
    echo -e "\n${BLUE}============================================${NC}"
    echo -e "${BLUE}$1${NC}"
    echo -e "${BLUE}============================================${NC}\n"
}

print_success() {
    echo -e "${GREEN}✅ $1${NC}"
}

print_warning() {
    echo -e "${YELLOW}⚠️  $1${NC}"
}

print_error() {
    echo -e "${RED}❌ $1${NC}"
}

print_info() {
    echo -e "${BLUE}ℹ️  $1${NC}"
}

# Banner
clear
echo -e "${RED}"
cat << "EOF"
   ____      _     _         _   _             ____  _             
  / ___|__ _| |__ | | ___   | | | |_ __   ___ |  _ \| | __ _ _   _ 
 | |   / _` | '_ \| |/ _ \  | | | | '_ \ / _ \| |_) | |/ _` | | | |
 | |__| (_| | |_) | |  __/  | |_| | | | | (_) |  __/| | (_| | |_| |
  \____\__,_|_.__/|_|\___|   \___/|_| |_|\___/|_|   |_|\__,_|\__, |
                                                               |___/ 
EOF
echo -e "${NC}"
echo -e "${BLUE}Instalación Automática en Servidor Linux${NC}\n"

# Verificar que se ejecuta como root o con sudo
if [[ $EUID -ne 0 ]]; then
   print_error "Este script debe ejecutarse con sudo"
   echo "Uso: sudo bash install-server.sh"
   exit 1
fi

# Obtener usuario real (no root)
REAL_USER="${SUDO_USER:-$USER}"
REAL_HOME=$(eval echo ~$REAL_USER)

print_header "📋 CONFIGURACIÓN INICIAL"

# Preguntar configuración
read -p "📧 Tu correo electrónico (para SSL): " USER_EMAIL
read -p "🌐 Tu dominio (ej: cableuno.com): " DOMAIN
read -p "🗄️  Contraseña para base de datos PostgreSQL: " -s DB_PASSWORD
echo ""
read -p "📂 ¿Tienes repositorio Git del proyecto? (s/n): " HAS_GIT

if [[ "$HAS_GIT" == "s" || "$HAS_GIT" == "S" ]]; then
    read -p "🔗 URL del repositorio Git: " GIT_REPO
    USE_GIT=true
else
    USE_GIT=false
    print_warning "Deberás subir el proyecto manualmente después"
fi

# Confirmar
echo ""
print_info "Configuración:"
echo "  - Email: $USER_EMAIL"
echo "  - Dominio: $DOMAIN"
echo "  - Git: $USE_GIT"
echo ""
read -p "¿Continuar con la instalación? (s/n): " CONFIRM

if [[ "$CONFIRM" != "s" && "$CONFIRM" != "S" ]]; then
    print_warning "Instalación cancelada"
    exit 0
fi

#==============================================================================
# PASO 1: Actualizar Sistema
#==============================================================================

print_header "🔄 ACTUALIZANDO SISTEMA"

apt update
apt upgrade -y
apt install -y curl wget git build-essential software-properties-common ufw

print_success "Sistema actualizado"

#==============================================================================
# PASO 2: Instalar Node.js 20
#==============================================================================

print_header "📦 INSTALANDO NODE.JS 20"

# Detectar si ya está instalado
if command -v node &> /dev/null; then
    NODE_VERSION=$(node --version)
    print_warning "Node.js ya instalado: $NODE_VERSION"
    read -p "¿Reinstalar? (s/n): " REINSTALL_NODE
    if [[ "$REINSTALL_NODE" == "s" || "$REINSTALL_NODE" == "S" ]]; then
        curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
        apt install -y nodejs
    fi
else
    curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
    apt install -y nodejs
fi

NODE_VERSION=$(node --version)
NPM_VERSION=$(npm --version)

print_success "Node.js $NODE_VERSION instalado"
print_success "npm $NPM_VERSION instalado"

#==============================================================================
# PASO 3: Instalar PostgreSQL
#==============================================================================

print_header "🗄️  INSTALANDO POSTGRESQL"

if command -v psql &> /dev/null; then
    print_warning "PostgreSQL ya instalado"
else
    apt install -y postgresql postgresql-contrib
fi

systemctl start postgresql
systemctl enable postgresql

print_success "PostgreSQL instalado y corriendo"

#==============================================================================
# PASO 4: Configurar Base de Datos
#==============================================================================

print_header "⚙️  CONFIGURANDO BASE DE DATOS"

DB_NAME="cableuno_play"
DB_USER="cableuno"

# Crear base de datos y usuario
sudo -u postgres psql -c "DROP DATABASE IF EXISTS $DB_NAME;" 2>/dev/null || true
sudo -u postgres psql -c "DROP USER IF EXISTS $DB_USER;" 2>/dev/null || true
sudo -u postgres psql -c "CREATE DATABASE $DB_NAME;"
sudo -u postgres psql -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASSWORD';"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;"
sudo -u postgres psql -c "ALTER DATABASE $DB_NAME OWNER TO $DB_USER;"

print_success "Base de datos '$DB_NAME' creada"
print_success "Usuario '$DB_USER' creado"

#==============================================================================
# PASO 5: Instalar PM2
#==============================================================================

print_header "🔧 INSTALANDO PM2"

npm install -g pm2

print_success "PM2 instalado"

#==============================================================================
# PASO 6: Configurar Proyecto
#==============================================================================

print_header "📂 CONFIGURANDO PROYECTO"

PROJECT_DIR="/var/www/cable-uno-play"

# Crear directorio
mkdir -p $PROJECT_DIR
chown -R $REAL_USER:$REAL_USER $PROJECT_DIR

if [[ "$USE_GIT" == true ]]; then
    print_info "Clonando repositorio..."
    
    # Si existe, hacer backup
    if [ -d "$PROJECT_DIR/.git" ]; then
        print_warning "Proyecto ya existe, actualizando..."
        cd $PROJECT_DIR
        sudo -u $REAL_USER git pull origin main || sudo -u $REAL_USER git pull origin master
    else
        # Clonar
        sudo -u $REAL_USER git clone $GIT_REPO $PROJECT_DIR
    fi
    
    print_success "Repositorio clonado/actualizado"
else
    print_warning "Proyecto NO clonado (no Git repo)"
    print_info "Sube el proyecto manualmente a: $PROJECT_DIR"
    print_info "Puedes usar: scp -r proyecto/ usuario@servidor:$PROJECT_DIR"
    
    read -p "¿Ya subiste el proyecto? (s/n): " PROJECT_READY
    if [[ "$PROJECT_READY" != "s" && "$PROJECT_READY" != "S" ]]; then
        print_error "Sube el proyecto primero y ejecuta el script nuevamente"
        exit 1
    fi
fi

cd $PROJECT_DIR

# Crear archivo .env
print_info "Creando archivo .env..."

cat > .env << EOL
# Base de datos
DATABASE_URL=postgresql://$DB_USER:$DB_PASSWORD@localhost:5432/$DB_NAME
PGHOST=localhost
PGPORT=5432
PGUSER=$DB_USER
PGPASSWORD=$DB_PASSWORD
PGDATABASE=$DB_NAME

# Node
NODE_ENV=production
PORT=5000
HOST=0.0.0.0
EOL

chown $REAL_USER:$REAL_USER .env
chmod 600 .env

print_success "Archivo .env creado"

# Instalar dependencias
print_info "Instalando dependencias (esto puede tardar)..."
sudo -u $REAL_USER npm install

print_success "Dependencias instaladas"

# Build de producción
print_info "Compilando proyecto..."
sudo -u $REAL_USER npm run build

print_success "Proyecto compilado"

# Sincronizar base de datos
print_info "Creando tablas en base de datos..."
sudo -u $REAL_USER npm run db:push || true

print_success "Base de datos sincronizada"

#==============================================================================
# PASO 7: Configurar PM2
#==============================================================================

print_header "🚀 CONFIGURANDO PM2"

# Detener si ya está corriendo
pm2 delete cable-uno-play 2>/dev/null || true

# Iniciar aplicación
cd $PROJECT_DIR
sudo -u $REAL_USER pm2 start dist/index.js --name cable-uno-play

# Guardar configuración
sudo -u $REAL_USER pm2 save

# Configurar inicio automático
sudo -u $REAL_USER pm2 startup systemd -u $REAL_USER --hp $REAL_HOME
env PATH=$PATH:/usr/bin pm2 startup systemd -u $REAL_USER --hp $REAL_HOME

print_success "PM2 configurado (app corriendo 24/7)"

#==============================================================================
# PASO 8: Instalar Nginx
#==============================================================================

print_header "🌐 INSTALANDO NGINX"

if command -v nginx &> /dev/null; then
    print_warning "Nginx ya instalado"
else
    apt install -y nginx
fi

systemctl start nginx
systemctl enable nginx

print_success "Nginx instalado"

#==============================================================================
# PASO 9: Configurar Nginx
#==============================================================================

print_header "⚙️  CONFIGURANDO NGINX"

NGINX_CONFIG="/etc/nginx/sites-available/cable-uno-play"

cat > $NGINX_CONFIG << 'EOL'
server {
    listen 80;
    server_name DOMAIN_PLACEHOLDER;

    access_log /var/log/nginx/cableuno_access.log;
    error_log /var/log/nginx/cableuno_error.log;

    location / {
        proxy_pass http://localhost:5000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
        
        proxy_connect_timeout 300s;
        proxy_send_timeout 300s;
        proxy_read_timeout 300s;
    }

    client_max_body_size 100M;
}
EOL

# Reemplazar dominio
sed -i "s/DOMAIN_PLACEHOLDER/$DOMAIN/g" $NGINX_CONFIG

# Habilitar sitio
ln -sf $NGINX_CONFIG /etc/nginx/sites-enabled/cable-uno-play

# Eliminar default si existe
rm -f /etc/nginx/sites-enabled/default

# Probar configuración
nginx -t

# Reiniciar Nginx
systemctl restart nginx

print_success "Nginx configurado para: $DOMAIN"

#==============================================================================
# PASO 10: Configurar SSL (HTTPS)
#==============================================================================

print_header "🔒 CONFIGURANDO SSL/HTTPS"

# Instalar Certbot
apt install -y certbot python3-certbot-nginx

print_info "Obteniendo certificado SSL..."
print_warning "Asegúrate que el dominio $DOMAIN apunte a este servidor"

read -p "¿Continuar con SSL? (s/n): " INSTALL_SSL

if [[ "$INSTALL_SSL" == "s" || "$INSTALL_SSL" == "S" ]]; then
    certbot --nginx -d $DOMAIN --non-interactive --agree-tos --email $USER_EMAIL --redirect
    
    print_success "SSL configurado - HTTPS activo"
    
    # Configurar auto-renovación
    systemctl enable certbot.timer
    systemctl start certbot.timer
    
    print_success "Auto-renovación de SSL configurada"
else
    print_warning "SSL omitido - Configúralo después con: sudo certbot --nginx -d $DOMAIN"
fi

#==============================================================================
# PASO 11: Configurar Firewall
#==============================================================================

print_header "🔥 CONFIGURANDO FIREWALL"

# Habilitar UFW si no está activo
if ufw status | grep -q "inactive"; then
    # Configurar reglas básicas
    ufw default deny incoming
    ufw default allow outgoing
    
    # Permitir SSH
    ufw allow ssh
    ufw allow 22
    
    # Permitir HTTP/HTTPS
    ufw allow 80/tcp
    ufw allow 443/tcp
    
    # Habilitar
    echo "y" | ufw enable
    
    print_success "Firewall configurado"
else
    print_warning "Firewall ya activo"
    ufw allow 80/tcp
    ufw allow 443/tcp
fi

ufw status

#==============================================================================
# PASO 12: Configurar Backups Automáticos
#==============================================================================

print_header "💾 CONFIGURANDO BACKUPS AUTOMÁTICOS"

BACKUP_SCRIPT="$REAL_HOME/backup-cableuno.sh"

cat > $BACKUP_SCRIPT << 'EOL'
#!/bin/bash
BACKUP_DIR="$HOME/backups/cableuno"
DATE=$(date +%Y%m%d_%H%M%S)
mkdir -p $BACKUP_DIR

# Backup de base de datos
pg_dump -U cableuno cableuno_play > "$BACKUP_DIR/db_backup_$DATE.sql"

# Mantener solo últimos 7 días
find $BACKUP_DIR -name "db_backup_*.sql" -mtime +7 -delete

echo "✅ Backup completado: db_backup_$DATE.sql"
EOL

chown $REAL_USER:$REAL_USER $BACKUP_SCRIPT
chmod +x $BACKUP_SCRIPT

# Agregar a crontab (diario a las 2 AM)
(sudo -u $REAL_USER crontab -l 2>/dev/null || echo "") | grep -v "backup-cableuno.sh" > /tmp/crontab.tmp
echo "0 2 * * * $BACKUP_SCRIPT" >> /tmp/crontab.tmp
sudo -u $REAL_USER crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp

print_success "Backups automáticos configurados (diario 2 AM)"

#==============================================================================
# PASO 13: Crear Script de Actualización
#==============================================================================

print_header "🔄 CREANDO SCRIPT DE ACTUALIZACIÓN"

UPDATE_SCRIPT="$PROJECT_DIR/update.sh"

if [[ "$USE_GIT" == true ]]; then
cat > $UPDATE_SCRIPT << 'EOL'
#!/bin/bash
set -e

echo "🔄 Actualizando Cable Uno Play..."

cd /var/www/cable-uno-play

echo "📥 Descargando cambios..."
git pull origin main || git pull origin master

echo "📦 Instalando dependencias..."
npm install

echo "🏗️  Compilando..."
npm run build

echo "🗄️  Actualizando base de datos..."
npm run db:push

echo "🔄 Reiniciando aplicación..."
pm2 restart cable-uno-play

echo "✅ ¡Actualización completada!"
pm2 status
EOL
else
cat > $UPDATE_SCRIPT << 'EOL'
#!/bin/bash
set -e

echo "🔄 Actualizando Cable Uno Play..."

cd /var/www/cable-uno-play

echo "📦 Instalando dependencias..."
npm install

echo "🏗️  Compilando..."
npm run build

echo "🗄️  Actualizando base de datos..."
npm run db:push

echo "🔄 Reiniciando aplicación..."
pm2 restart cable-uno-play

echo "✅ ¡Actualización completada!"
pm2 status
EOL
fi

chmod +x $UPDATE_SCRIPT
chown $REAL_USER:$REAL_USER $UPDATE_SCRIPT

print_success "Script de actualización creado: $UPDATE_SCRIPT"

#==============================================================================
# FINALIZACIÓN
#==============================================================================

print_header "🎉 ¡INSTALACIÓN COMPLETADA!"

echo ""
print_success "Cable Uno Play está instalado y corriendo"
echo ""

if [[ "$INSTALL_SSL" == "s" || "$INSTALL_SSL" == "S" ]]; then
    echo -e "${GREEN}🌐 Tu app está disponible en:${NC}"
    echo -e "   ${BLUE}https://$DOMAIN${NC}"
else
    echo -e "${YELLOW}🌐 Tu app está disponible en:${NC}"
    echo -e "   ${BLUE}http://$DOMAIN${NC}"
    echo -e "   ${YELLOW}(Configura SSL para HTTPS)${NC}"
fi

echo ""
echo -e "${BLUE}📊 Información del Sistema:${NC}"
echo "   - Proyecto: $PROJECT_DIR"
echo "   - Base de datos: $DB_NAME"
echo "   - Usuario DB: $DB_USER"
echo "   - Logs: /var/log/nginx/"

echo ""
echo -e "${BLUE}📋 Comandos Útiles:${NC}"
echo "   Ver estado:     pm2 status"
echo "   Ver logs:       pm2 logs cable-uno-play"
echo "   Reiniciar:      pm2 restart cable-uno-play"
echo "   Actualizar:     $UPDATE_SCRIPT"
echo "   Nginx logs:     sudo tail -f /var/log/nginx/cableuno_error.log"

echo ""
echo -e "${GREEN}✅ Todo listo para producción!${NC}"
echo ""

# Mostrar estado final
pm2 status
